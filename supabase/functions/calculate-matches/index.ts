...
async function findMatchesForJob(jobId: string, supabase: any) {
  // 1. Get job details
...
async function findJobsForMember(profileId: string, supabase: any) {
...
function calculateSkillsScore(memberSkills: any[], requiredSkills: string[], preferredSkills: string[] = []): number {
...
function calculateInterestScore(preferences: any, jobIndustry: string, jobType: string): number {
...
function calculateSalaryScore(preferences: any, jobMin: number, jobMax: number): number {
...
function generateMatchReason(skills: number, interests: number, location: number, salary: number, availability: number): string {
  const reasons = []
